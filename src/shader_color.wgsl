...
struct VertexOutput {
    ...
    @location(0) world_position: vec2<f32>, // World position to be send to fragment shader
};
...
fn vs_main(
    ...
    // This will be interpolated across the triangule surface
    out.world_position = vec2<f32>(x, y);
    return out;
}
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let normalized_position = in.world_position + 0.5;
